...
- **Directly Observed**: Sounding pitch-class simultaneities at note onsets.
...
- **Directly Observed**: Onset time intervals, note duration lengthening, rest occurrences, lowest sounding pitch (bass), metric measure positions.
...
- **Directly Observed**: Measure-level role-blind feature vectors (pitch-class distribution, register, density).
...
- **Literature Reference**:
  ...
  - Huron, D. (2001). *Tone and Voice: A Perceptual Theory of Voice Leading*. MIT Press.
...
- **Inferred Proxy**: Motion classification between successive outer-voice pairs: Parallel ($\Delta p_1 \cdot \Delta p_2 > 0$ with $|\Delta p_1| = |\Delta p_2|$), Contrary ($\Delta p_1 \cdot \Delta p_2 < 0$), Oblique ($\Delta p_1 = 0 \text{ xor } \Delta p_2 = 0$), Similar ($\Delta p_1 \cdot \Delta p_2 > 0$).
...
- **Literature Reference**:
  ...
  - Huron, D. (1989). *Voice denumerability in polyphonic music*. Contemporary Music Review.
- **Directly Observed**: Exact MIDI pitches, note onset simultaneities, intra-measure onset intervals, staff distribution.
- **Inferred Proxy**:
  - Arpeggiation proxy: rapid consecutive monophonic or thin attacks within a measure forming a coherent pitch-class sonority across a wide registral span.
...
- **Known Failure Modes**: Pieces with abrupt formal interruptions or irregular tempo/measure structures can exhibit step-like rather than smooth trajectory changes.
