...
Cryptographically binds cell values, availability statuses, and reason strings.
"""
...
class StructuralRepresentationMatrix:
    ...
    reasons_matrix: tuple[tuple[str, ...], ...]
    manifest_hash: str
    schema_hash: str
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.reasons_matrix) != len(self.piece_ids):
            raise ValueError("reasons_matrix row count must match piece_ids length.")
    ...
    def compute_matrix_hash(self) -> str:
        """
        Deterministic SHA-256 hash of complete structural matrix, availability states, and reason strings.
        """
        canonical = {
                    ...
                    "cells": [
                        {
                            "feature_id": fname,
                            "value": val,
                            "availability_status": st.value,
                            "availability_reason": rsn,
                        }
                        for fname, val, st, rsn in zip(
                            self.feature_names, row, avail_row, reason_row, strict=True
                        )
                    ],
                }
                for pid, row, avail_row, reason_row in zip(
                    self.piece_ids, self.data, self.availability_matrix, self.reasons_matrix, strict=True
                )
            ],
        }
...
def build_structural_representation_matrix(
    ...
    return StructuralRepresentationMatrix(
        ...
        reasons_matrix=tuple(reason_rows),
