"""Script to verify end-to-end reproducibility of feature extraction across independent subprocesses.

This script executes two completely independent feature extraction runs starting from
fresh Python process invocations (subprocess.run), writes output to isolated temporary
directories, and compares all piece IDs, canonical piece hashes, column IDs, feature values,
policy hashes, schema semantic hashes, and matrix semantic hashes.
"""
...
def run_single_extraction_subprocess(output_dir: Path) -> dict:
    """Launch a fresh Python subprocess to execute run_extraction."""
    ...
    if proc.returncode != 0:
        print(f"Worker subprocess failed:\n{proc.stderr}")
        sys.exit(proc.returncode)

    summary_path = output_dir / "summary.json"
    if not summary_path.exists():
        print(f"ERROR: Worker output {summary_path} not found.")
        sys.exit(1)

    return json.loads(summary_path.read_text(encoding="utf-8"))


def worker_main(output_dir_str: str) -> None:
    """Worker entrypoint executing in an independent Python process."""
    ...
    schema_hash = compute_schema_semantic_hash(FEATURE_REGISTRY)
...
def main() -> None:
    ...
    print("--- Running Feature Reproducibility Verification (Two Fresh Subprocesses) ---")
    ...
    try:
        ...
        res1 = run_single_extraction_subprocess(tmp_run1)
        print(f"Run 1 Schema Semantic Hash: {res1['feature_schema_semantic_hash']}")
        print(f"Run 1 Policy Hash:          {res1['feature_policy_hash']}")
        print(f"Run 1 Matrix Hash:          {res1['matrix_hash']}")

        print(f"\nRun 2 output destination: {tmp_run2}")
        res2 = run_single_extraction_subprocess(tmp_run2)
        print(f"Run 2 Schema Semantic Hash: {res2['feature_schema_semantic_hash']}")
        ...
        schema_mismatch = 1 if res1["feature_schema_semantic_hash"] != res2["feature_schema_semantic_hash"] else 0
        ...
        print("\n--- Two-Process Reproducibility Comparison Results ---")
        print(f"Row mismatches:            {row_mismatch}")
        print(f"Column mismatches:         {col_mismatch}")
        print(f"Value mismatches:          {val_mismatch}")
        print(f"Schema hash mismatches:    {schema_mismatch}")
        print(f"Policy hash mismatches:    {policy_mismatch}")
        print(f"Matrix hash mismatches:    {matrix_mismatch}")

        if any([row_mismatch, col_mismatch, val_mismatch, schema_mismatch, policy_mismatch, matrix_mismatch]):
            print("\nResult: TWO-PROCESS REPRODUCIBILITY FAILED")
            sys.exit(1)
        else:
            print("\nResult: 100% PERFECT TWO-PROCESS REPRODUCIBILITY VERIFIED")
            sys.exit(0)
    finally:
